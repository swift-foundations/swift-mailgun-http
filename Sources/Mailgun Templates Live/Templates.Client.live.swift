  ...
  public static func live(
    ...
    return Self(
      ...
      createVersion: { templateName, request in
        try await handleRequest(
          for: makeRequest(
            .createVersion(domainId: domain, templateName: templateName, request: request)
          ),
      ...
      getVersion: { templateName, versionName in
        try await handleRequest(
          for: makeRequest(
            .getVersion(domainId: domain, templateName: templateName, versionName: versionName)
          ),
      ...
      deleteVersion: { templateName, versionName in
        try await handleRequest(
          for: makeRequest(
            .deleteVersion(domainId: domain, templateName: templateName, versionName: versionName)
          ),
