...
struct MailgunSubaccountsTests {
  ...
  func testCreateAndDeleteSubaccount() async throws {
    ...
    do {
      ...
      #expect(
        createResponse.subaccount.status == .open || createResponse.subaccount.status == .disabled
      )
      ...
      #expect(
        deleteResponse.message.contains("deleted") || deleteResponse.message.contains("success")
      )
