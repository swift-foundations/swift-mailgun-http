...
struct MailgunSuppressionsComplaintsTests {
  ...
  func testCreateAndDeleteComplaint() async throws {
    ...
    do {
      ...
      #expect(
        deleteResponse.message.contains("removed") || deleteResponse.message.contains("deleted")
      )
  ...
  func testComplaintNotFound() async throws {
    ...
    do {
      ...
      #expect(
        String(describing: error).contains("404") || String(describing: error).contains("not found")
      )
    }
  }
