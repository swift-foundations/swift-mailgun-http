...
struct MailgunCredentialsTests {
  ...
  func testUpdateMailbox() async throws {
    ...
    do {
      ...
      #expect(
        updateResponse.message.contains("Updated") || updateResponse.message.contains("modified")
          || updateResponse.message.contains("Password")
      )
    } catch {
  ...
  func testCredentialWithSpecialCharacters() async throws {
    ...
    do {
      ...
      #expect(
        String(describing: error).contains("sandbox") || String(describing: error).contains("400"),
